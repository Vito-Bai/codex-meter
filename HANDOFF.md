...
- 当前应用使用 ad-hoc 签名，公开分发前必须使用 Developer ID 签名并完成 Apple 公证。
...
- 面板核心分区：本周期剩余、使用节奏、最近一轮消耗、今日/周期用量、Credits/重置信息。
...
```bash
git clone <PRIVATE_REPOSITORY_URL> codex-meter
cd codex-meter
swift build
```
...
2. 完善设置持久化和开机启动。
3. 为 App Server 协议变化增加兼容测试与用户可理解的错误提示。
4. 使用 Instruments 持续验证快速滚动、内存和后台能耗。
5. 评估正式更新通道；复盘分析能力确认价值后再扩展，避免重新堆叠信息。
